...
public class PrecioNegativoServicioTest {
    ...
    public void verificarValidacionPrecioNegativoServicio() throws Exception {
        ...
        userField.clear();
        userField.sendKeys("adminServicios"); // Usuario ADMIN_USUARIOS con permisos de servicio
        ...
        // Esperar a que aparezca un mensaje de validación relacionado con precio
        // Busca variantes de mensajes de error (case-insensitive, múltiples opciones)
        By errorMensaje = By.xpath(
            "//*[" +
            ...
            "contains(translate(., 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz'), 'precio') and contains(translate(., 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz'), 'positivo') or " +
            "contains(translate(., 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz'), 'número positivo')" +
            "]"
        );

        WebElement mensajeError = wait.until(
            ExpectedConditions.visibilityOfElementLocated(errorMensaje)
        );

        Assert.assertTrue(mensajeError.isDisplayed(), 
            "No apareció el mensaje de error de validación para precio negativo");
